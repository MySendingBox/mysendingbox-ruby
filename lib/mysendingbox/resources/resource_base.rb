  ...
  module Resources
    class ResourceBase
      ...
      def submit(method, url, parameters={}, headers={})
        clientVersion = Mysendingbox::VERSION
        ...
        begin
          if method == :get || method == :delete
            ...
            response = RestClient.send(method, url, {
              user_agent: 'Mysendingbox/v1 RubyBindings/' + clientVersion,
              "Mysendingbox-Version" => config[:api_version]
            })
          else
      ...
      def handle_api_error(error)
        begin
          ...
          # :nocov:
          raise MysendingboxError.new("Invalid response object:", error.http_code, error.http_body)
          # :nocov:
        end
      end
      ...
      def base_url
        "https://#{config[:api_key]}:@api.mysendingbox.fr"
      end
